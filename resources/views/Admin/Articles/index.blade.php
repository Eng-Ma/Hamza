        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    <a href="{{ route('admin.articles.create') }}" class="btn btn-success">Add Articles</a>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($articles as $article)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($article->image)
                                                <img src="{{ asset('uploads/image/articles/' . $article->image) }}" alt="Page Image"
                                        ...
                                        <td>
                                            @if ($article->status == StatusEnum::ACTIVE)
                                                <a href="{{ route('admin.articles.toggle_active', ['id' => $article->id]) }}"
                                                    class="btn text-white" style="background: #4FE39C">
                                                    <i class="fa fa-check"></i>
                                                </a>
                                            @elseif($article->status == StatusEnum::INACTIVE)
                                                <a href="{{ route('admin.articles.toggle_active', ['id' => $article->id]) }}"
                                        ...
                                        <td>
                                            ...
                                            <form class="d-inline"
                                                action="{{ route('admin.articles.delete', ['id' => $article->id]) }}"
                                                method="POST">
                                                @csrf
