        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    <a href="{{ route('admin.contacts.create') }}" class="btn btn-success">Add Contacts</a>
                </div>
                <div class="card-body">
                        ...
                        <table id="file-datatable" class="table table-bordered text-nowrap key-buttons">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">order_link</th>
                                    <th class="border-bottom-0">info_link</th>
                            ...
                            <tbody>
                                @foreach ($contacts as $about)
                                    <tr>
                                        <td>{{ $about->id }}</td>
                                        <td>{{ $about->questions }}</td>
                                        <td>
                                            @if ($about->image)
                                                <img src="{{ asset('uploads/image/contact/' . $about->image) }}" alt="Page Image"
                                        ...
                                        <td>{{ $about->order_link }}</td>
                                        <td>{{ $about->info_link }}</td>
                                        ...
                                        <td>
                                            @if ($about->status == StatusEnum::ACTIVE)
                                                <a href="{{ route('admin.contacts.toggle_active', ['id' => $about->id]) }}"
                                                    class="btn text-white" style="background: #4FE39C">
                                                    <i class="fa fa-check"></i>
                                                </a>
                                            @elseif($about->status == StatusEnum::INACTIVE)
                                                <a href="{{ route('admin.contacts.toggle_active', ['id' => $about->id]) }}"
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-primary"
                                                href="{{ route('admin.contacts.edit', $about->id) }}">
                                                <span class="fe fe-edit"></span>
                                            </a>
                                            <form class="d-inline"
                                                action="{{ route('admin.contacts.delete', ['id' => $about->id]) }}"
                                                method="POST">
                                                @csrf
