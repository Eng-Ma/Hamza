                        ...
                        <table id="file-datatable" class="table table-bordered text-nowrap key-buttons">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">Category_id</th>
                                    ...
                                    <th class="border-bottom-0">package_size</th>
                                    <th class="border-bottom-0">main_components</th>
                            ...
                            <tbody>
                                @foreach ($products as $product)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($product->image)
                                                <img src="{{ asset('uploads/image/products/' . $product->image) }}" alt="Page Image"
                                        ...
                                        <td>{{ $product->main_components }}</td>
                                        <td>{{ $product->description }}</td>


                                        <td>
                                            @if ($product->status == StatusEnum::ACTIVE)
                                                <a href="{{ route('admin.products.toggle_active', ['id' => $product->id]) }}"
                                                    class="btn text-white" style="background: #4FE39C">
                                                    <i class="fa fa-check"></i>
                                                </a>
                                            @elseif($product->status == StatusEnum::INACTIVE)
                                                <a href="{{ route('admin.products.toggle_active', ['id' => $product->id]) }}"
